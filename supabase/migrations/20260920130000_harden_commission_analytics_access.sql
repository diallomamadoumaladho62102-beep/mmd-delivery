-- P0/P1 hardening:
-- 1) refresh_order_commissions: block arbitrary authenticated callers from rewriting
--    any order's commissions (trigger path still works for order participants).
-- 2) analytics_daily_metrics: staff-only SELECT (was USING true).
-- Does NOT modify older migration files.
...
begin
  ...
  -- Authorization: service_role, staff, or order participant (trigger path).
...
-- Analytics metrics contain revenue-style snapshots — staff only.
