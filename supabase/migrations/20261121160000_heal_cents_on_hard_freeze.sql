-- Hard-freeze (transfer ids present) must still heal *_cents / fee_* from
-- authoritative *_amount. Observed after Food E2E Connect transfer:
-- amounts stayed 2.97/5.36/4.25 but cents briefly drifted to 339/651/200
-- (legacy fee pattern). Wallet SoT prefers *_cents → must realign even when
-- money is frozen.
--
-- Does NOT recompute from pricing_config. Idempotent.
...
begin
  ...
  if v_has_existing
    ...
    v_freeze_reason := 'transfer_ids_present';
  elsif v_has_existing
     and v_has_snap
     and lower(trim(coalesce(v_order.payment_status::text, ''))) = 'paid' then
    v_freeze_reason := 'paid_with_snapshot';
  end if;

  -- Soft or hard freeze: never recompute money; only heal cents mirrors from amounts.
  if v_freeze_reason is not null then
