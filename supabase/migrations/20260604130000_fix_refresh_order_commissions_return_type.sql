-- Production: replace 3 RPC signatures (void/boolean/integer -> jsonb).
-- NEVER drop: refresh_order_commissions(), refresh_order_commissions_trg() (triggers).
...
drop function if exists public.refresh_order_commissions_rpc(uuid);
drop function if exists public.refresh_order_commissions_for_range(timestamptz, timestamptz);
drop function if exists public.refresh_order_commissions(uuid);
