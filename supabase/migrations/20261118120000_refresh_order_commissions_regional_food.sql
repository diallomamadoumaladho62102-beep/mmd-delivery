-- Align refresh_order_commissions pricing_config selection with quote SoT
-- (apps/web/src/lib/platformCountryInference.ts pricingConfigKeyForOrder).
-- Food/errand Africa markets use food_africa / errand_africa; else *_default.
--
-- Freeze rules (money safety):
-- 1) commission_snapshots.rate_pct freezes restaurant/platform % when present.
-- 2) orders.delivery_pay freezes driver share when present.
-- 3) Once order_commissions exists AND payment is paid AND a snapshot exists,
--    do NOT rewrite amounts (pricing_config live changes must not mutate paid orders).
-- 4) Once restaurant_transfer_id or driver_transfer_id is set, never rewrite.
--
-- food_us remains unused by quote + this RPC (seeded historically; not product SoT).
-- Does NOT drop data. No destructive table clears. No row deletes of business data.
-- Rollback: restore function body from 20260920130000_harden_commission_analytics_access.sql
