-- Production hotfixes:
-- 1) driver_accept_order_offer: cast orders.kind (enum order_kind) to text before lower()
-- 2) Ensure public.profiles columns used by mobile profile updates exist (PostgREST schema cache)

begin;

-- ---------------------------------------------------------------------------
-- profiles: columns expected by DriverProfileScreen / avatar upserts
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- driver_accept_order_offer: lower(order_kind) does not exist without ::text
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Production: orders.kind may be enum order_kind — cast before lower().
...
-- Ask PostgREST to reload schema cache after column additions.
