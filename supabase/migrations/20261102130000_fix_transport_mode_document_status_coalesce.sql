-- Fix Sentry MMD-DELIVERY-WEB-5 / MOBILE document_status "" errors.
-- change_driver_transport_mode used: lower(coalesce(dd.status, ''))
-- When status was enum document_status, coalesce(..., '') cast '' to the enum → 22P02.
...
begin
  ...
  select exists (
    ...
    where dd.user_id = p_user_id
      ...
      and lower(dd.status::text) in ('approved', 'verified', 'valid')
  )
  or (
