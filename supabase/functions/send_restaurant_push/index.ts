...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!serviceKey || !supabaseUrl) {
      console.error("[send_restaurant_push] missing Supabase env");
      return json({ ok: false, error: "Server misconfigured" }, 500);
    }

    if (!pushKey && !String(Deno.env.get("CRON_SECRET") ?? "").trim()) {
      console.error("[send_restaurant_push] missing PUSH_API_KEY or CRON_SECRET");
      return json({ ok: false, error: "Server misconfigured" }, 500);
    }
    ...
    if (tokenError) {
      console.error("[send_restaurant_push] token lookup failed", {
    ...
    console.error("[send_restaurant_push] unhandled", {
