#!/usr/bin/env node
/**
 * Apply ONLY 20261012120000_food_delivery_checkout_intents_pay_then_create.sql to linked prod.
...
const MIG_VERSION = "20261012120000";
const MIG_NAME = "food_delivery_checkout_intents_pay_then_create";
const MIG_FILE = join(
  ROOT,
  "supabase",
  "migrations",
...
const BACKUP = join(ROOT, "backups", `supabase-${PROJECT_REF}-food-delivery-intents-${STAMP}`);
...
try {
  ...
  const verify = dbQuery(
    `
select
  (select to_regclass('public.food_checkout_intents') is not null) as food_table_exists,
  (select to_regclass('public.delivery_checkout_intents') is not null) as delivery_table_exists,
  (select count(*) from information_schema.columns
    where table_schema='public' and table_name='food_checkout_intents') as food_column_count,
  (select count(*) from information_schema.columns
    where table_schema='public' and table_name='delivery_checkout_intents') as delivery_column_count,
...
`,
  ...
  if (/error|does not exist|syntax error/i.test(verify) && !/food_table_exists/i.test(verify)) {
