...
export default function CreateOrderPage() {
  ...
  const createOrder = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('orders')
        .insert({ client_id: uid, restaurant_id: 'demo-restaurant', total_amount: 16.00, delivery_fee: 3.00 })
        .select('id')
        .single();
      if (error) throw error;
      setOrderId(data.id);
  ...
  return (
    <main className="p-6 space-y-4">
      ...
      {orderId && <p className="mt-2">Commande créée ✅ → <Link className="underline" href={`/orders/${orderId}/chat`}>ouvrir le chat</Link></p>}
