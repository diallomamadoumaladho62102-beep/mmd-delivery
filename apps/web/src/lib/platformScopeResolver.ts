...
export async function fetchPlatformScopeConfig(
  ...
  if (applyRegion) {
    if (scope.mmd_zone_id) {
      ...
      if (byZone) regionConfig = regionRowToToggleConfig(byZone);
    }
    ...
    if (!regionConfig && scope.region_code) {
      ...
      if (byRegion) regionConfig = regionRowToToggleConfig(byRegion);
    }
  }

  return applyCountryFloor(countryConfig, regionConfig, applyRegion);
}
...
export async function resolveDriverPlatformScope(
  ...
  if (missionCountry.length === 2) {
    return buildScopeKey({
      ...
      scopeLevel: input.missionMmdZoneId ? "zone" : input.missionRegionCode ? "region" : "country",
      source: "mission",
    });
  }
...
function buildScopeKey(params: {
  ...
  const scope_level =
    params.scopeLevel ??
    (params.mmdZoneId ? "zone" : region_code ? "region" : "country");
...
): Promise<PlatformFeatureResult & { region_code?: string; scope_level?: string }> {
  const config = await fetchPlatformScopeConfig(supabase, scope);
  if (!config) {
...
export async function assertPlatformFeatureWithScope(
  ...
  vertical: PlatformVertical,
  feature: PlatformFeature,
  scope?: Pick<PlatformScopeKey, "region_code" | "mmd_zone_id"> | null
  ...
  if (scope?.region_code || scope?.mmd_zone_id) {
