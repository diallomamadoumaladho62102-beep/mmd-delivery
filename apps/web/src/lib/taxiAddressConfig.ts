...
function asMeters(value: unknown, fallback: number): number {
...
export function taxiAddressConfigSeedMetadata(countryCode: string): Record<string, unknown> {
  return {
    address_config: defaultTaxiAddressConfig(countryCode),
  };
}
