...
export async function openDeliveryQuoteNativePaymentIntent(params: {
  supabaseAdmin: SupabaseClient;
  intentId: string;
  userId: string;
  snapshot: DeliveryCheckoutIntentSnapshot;
  ...
  const paymentIntent = await stripe.paymentIntents.create(
    {
      ...
      metadata: {
        ...
        amount_cents: String(amountCents),
        currency: currency.toUpperCase(),
        source_route: "create-delivery-quote-checkout-session:apple_pay",
      },
    },
    {
      idempotencyKey:
        `delivery_quote_pi_${params.intentId}_${amountCents}_${currency}`.slice(
          0,
          255,
        ),
    },
