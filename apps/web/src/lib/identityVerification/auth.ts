...
export function parseIdentitySubjectType(value: unknown): IdentitySubjectType | null {
  const v = String(value ?? "").trim().toLowerCase();
  return (IDENTITY_SUBJECT_TYPES as readonly string[]).includes(v)
    ? (v as IdentitySubjectType)
    : null;
}

export async function requireIdentityActor(req: NextRequest) {
