...
export default function RevenueChart({ from, to }: { from: string; to: string }) {
  ...
  if (loading) return <div className="text-sm text-gray-500">{t("Chargement du graphique…")}</div>;
  if (rows.length === 0) return <div className="text-sm text-gray-500">{t("Aucune donnée à afficher.")}</div>;
  ...
  return (
    <div className="space-y-2">
      <div className="text-xs text-gray-500">
        {t("Total plateforme par jour (NYC) • Max jour:")} <span className="font-semibold">{fmtMoney(points.yMax, points.cur)}</span>
      </div>
