...
import {
  DRIVERS_SEO,
  DRIVERS_START_STEPS,
  buildDriversFallbackBlocks,
} from "@/components/site/driversContent";
import {
  cmsPageMetadata,
  loadSiteChrome,
  renderCmsPage,
...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const meta = await cmsPageMetadata("drivers", "Drive with MMD");
    return {
      ...meta,
      title: meta.title || DRIVERS_SEO.title,
      description: meta.description || DRIVERS_SEO.description,
    };
  } catch {
    return {
      title: DRIVERS_SEO.title,
      description: DRIVERS_SEO.description,
      robots: DRIVERS_SEO.robots,
    };
  }
}
...
export default async function Page() {
  ...
  const pageData = await getPublishedPageBySlug(supabase, "drivers");
  ...
  const stepsBlock = pageData?.blocks.find((block) => block.block_type === "how_it_works");
  const payload =
  ...
  const jsonLd = (
    <HowToJsonLd
      name="How to start driving with MMD Delivery"
      description={DRIVERS_SEO.description}
      path="/drivers"
  ...
  return (
    <>
      ...
      >
        <BlockRenderer
          blocks={buildDriversFallbackBlocks()}
