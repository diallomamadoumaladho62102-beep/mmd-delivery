...
export default function CreateErrandPage() {
  ...
  return (
    <div className="max-w-md mx-auto p-6 border rounded-xl mt-6 space-y-3">
      <h1 className="text-xl font-bold">{t("Créer une commande (Errand)")}</h1>

      {!user && (
        <div className="p-3 border rounded-lg bg-yellow-50 text-sm">
          <div className="font-medium mb-1">{t("Non connecté")}</div>
          <div className="flex gap-4">
            <a className="underline text-blue-700" href="/auth/sign-in-password">
              {t("Se connecter (email + mot de passe)")}
            </a>
      ...
      <label className="block text-sm">{t("Adresse pickup")}</label>
      <input
      ...
      <label className="block text-sm">{t("Adresse dropoff")}</label>
      <input
        className="w-full border rounded px-3 py-2"
      ...
      <p className="text-sm text-gray-600">
        {t("Le montant est calculé côté serveur selon la configuration plateforme.")}
      </p>
