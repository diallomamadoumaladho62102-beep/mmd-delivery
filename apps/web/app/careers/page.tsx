...
import {
  CAREERS_SEO,
  CAREERS_START_STEPS,
  buildCareersFallbackBlocks,
} from "@/components/site/careersContent";
import {
  cmsPageMetadata,
  loadSiteChrome,
  renderCmsPage,
...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const meta = await cmsPageMetadata("careers", "Careers");
    return {
      ...meta,
      title: meta.title || CAREERS_SEO.title,
      description: meta.description || CAREERS_SEO.description,
    };
  } catch {
    return {
      title: CAREERS_SEO.title,
      description: CAREERS_SEO.description,
      robots: CAREERS_SEO.robots,
    };
  }
}
...
function readSteps(payload: Record<string, unknown> | undefined) {
  ...
  return fromCms.length > 0
    ? fromCms
    : CAREERS_START_STEPS.map((step) => ({
        title: step.title,
        body: step.body,
      }));
}

export default async function CareersMarketingPage() {
  const supabase = buildSupabaseAdminClient();
  const pageData = await getPublishedPageBySlug(supabase, "careers");
  ...
  return (
    <>
      ...
      >
        <BlockRenderer
          blocks={buildCareersFallbackBlocks()}
