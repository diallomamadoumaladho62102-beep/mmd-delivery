...
import {
  CONTACT_SEO,
  buildContactFallbackBlocks,
} from "@/components/site/contactContent";
import {
  cmsPageMetadata,
  loadSiteChrome,
  renderCmsPage,
...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const meta = await cmsPageMetadata("contact", "Contact");
    return {
      ...meta,
      title: meta.title || CONTACT_SEO.title,
      description: meta.description || CONTACT_SEO.description,
    };
  } catch {
    return {
...
export default async function ContactMarketingPage() {
  const supabase = buildSupabaseAdminClient();
  const pageData = await getPublishedPageBySlug(supabase, "contact");
  ...
  return (
    <>
      ...
      >
        <BlockRenderer
          blocks={buildContactFallbackBlocks()}
