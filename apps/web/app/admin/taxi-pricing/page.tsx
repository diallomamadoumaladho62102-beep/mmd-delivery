...
export default function AdminTaxiPricingPage() {
  ...
  async function onSave(e: FormEvent<HTMLFormElement>, row: TaxiPricingRow) {
    ...
    if (
      ...
      alert(t("Driver Share + MMD Platform Share must total ≤ 100%"));
      return;
    }
    ...
    <AdminGate requiredPermission="taxi_pricing.read">
      <main className="space-y-6">
        <div className="mx-auto max-w-6xl space-y-6">
          <header>
            <h1 className="text-2xl font-bold text-slate-900">{t("Taxi Pricing")}</h1>
          ...
          <div className="flex flex-wrap gap-4 rounded-2xl border border-slate-200 bg-white p-4">
            <label className="text-sm">
              <span className="mb-1 block text-slate-600">{t("Pays")}</span>
              <select
                value={countryFilter}
                onChange={(e) => {
            ...
            <label className="text-sm">
              <span className="mb-1 block text-slate-600">{t("Devise")}</span>
              <select
                value={currencyFilter}
                onChange={(e) => setCurrencyFilter(e.target.value)}
                className="rounded-xl border border-slate-300 px-3 py-2"
              >
                <option value="">{t("Toutes (pays sélectionné)")}</option>
          ...
          <section className="rounded-2xl border border-emerald-200 bg-emerald-50/70 p-5">
            ...
            <p className="mt-1 text-xs text-emerald-900/80">
              {t("Calcul via les moteurs live sur les tarifs")} <strong>sauvegardés</strong>.
              Enregistrez d&apos;abord vos changements pour les voir ici.
            </p>
            <div className="mt-4 flex flex-wrap gap-3">
              <label className="text-sm">
                <span className="mb-1 block text-slate-600">{t("Classe")}</span>
                <select
                  value={previewClass}
              ...
              <label className="text-sm">
                <span className="mb-1 block text-slate-600">{t("Miles")}</span>
                <input
                  type="number"
                  step="0.1"
                  min="0"
              ...
              <label className="text-sm">
                <span className="mb-1 block text-slate-600">{t("Minutes")}</span>
                <input
                  type="number"
                  step="1"
                  min="0"
            ...
            {preview ? (
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                <div className="rounded-xl bg-white p-4 text-sm shadow-sm">
                  <div className="font-semibold text-slate-900">{t("Client")}</div>
                  <dl className="mt-2 space-y-1 text-slate-700">
                    <div className="flex justify-between gap-3">
                      <dt>{t("Customer Fare")}</dt>
                ...
                >
                  ...
                  <div className="mt-4 space-y-3 rounded-xl border border-sky-100 bg-sky-50 p-3 text-sm">
                    <div className="font-semibold text-sky-950">
                      {t("Répartition (subtotal course)")}
                    </div>
                    <label className="block">
                      <span className="text-slate-600">{t("Driver Share (%)")}</span>
                      <input
                    ...
                    <p className="text-[11px] text-sky-900/70">
                      {t("Somme ≤ 100%. Appliqué au subtotal (hors tax &amp; service fee) par")} <code>quote_taxi_ride</code>.
                    </p>
                  </div>

                  <div className="mt-4 space-y-3 rounded-xl border border-indigo-100 bg-indigo-50 p-3 text-sm">
                    <div className="font-semibold text-indigo-900">
                      {t("Frais client (Service Fee)")}
                    </div>
                    ...
                    <label className="block">
                      <span className="text-slate-600">{t("Service fee %")}</span>
                      <input
