...
import {
  PARTNERS_SEO,
  PARTNERS_START_STEPS,
  buildPartnersFallbackBlocks,
} from "@/components/site/partnersContent";
import {
  cmsPageMetadata,
  loadSiteChrome,
  renderCmsPage,
...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const meta = await cmsPageMetadata("partners", "Partners");
    return {
      ...meta,
      title: meta.title || PARTNERS_SEO.title,
      description: meta.description || PARTNERS_SEO.description,
    };
  } catch {
    return {
      title: PARTNERS_SEO.title,
      description: PARTNERS_SEO.description,
      robots: PARTNERS_SEO.robots,
    };
  }
}
...
export default async function PartnersMarketingPage() {
  ...
  const pageData = await getPublishedPageBySlug(supabase, "partners");
  ...
  return (
    <>
      ...
      >
        <BlockRenderer
          blocks={buildPartnersFallbackBlocks()}
