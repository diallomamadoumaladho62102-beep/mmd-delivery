...
const BENEFIT_LABELS: Record<string, string> = {
  service_fee_credit: "Crédit sur frais de service",
  commission_discount: "Réduction de commission",
  sponsored_boost: "Mise en avant sponsorisée",
  priority_placement: "Placement prioritaire",
  recommended_badge: "Badge Restaurant recommandé",
  free_campaign: "Campagne promotionnelle gratuite",
  ad_credit: "Crédit publicitaire MMD",
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [summaryRes, rewardsRes] = await Promise.all([
        authFetch("/api/restaurant/loyalty/summary"),
        authFetch("/api/restaurant/loyalty/rewards"),
      ]);
  ...
  const redeem = useCallback(
    async (reward: Reward) => {
      ...
      try {
        ...
        const res = await authFetch("/api/restaurant/loyalty/redeem", {
          method: "POST",
