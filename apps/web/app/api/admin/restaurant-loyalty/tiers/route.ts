...
function buildTierPatch(
  ...
  for (const key of ["min_avg_rating", "min_acceptance_rate", "max_cancellation_rate"]) {
    if (body[key] !== undefined) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    await writeAdminAuditServer({
      ...
      action: "restaurant_loyalty_tier_created",
      targetType: "restaurant_loyalty_tier",
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    await writeAdminAuditServer({
      supabaseAdmin: supabase,
      adminUserId: session.userId,
      action: "restaurant_loyalty_tier_updated",
      targetType: "restaurant_loyalty_tier",
      targetId: id,
