...
/**
 * Manual seller loyalty operations, all requiring a reason and auditable:
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === "adjust_points") {
      ...
      const { data, error } = await supabase.rpc("mmd_marketplace_loyalty_admin_adjust", {
      ...
      await writeAdminAuditServer({
        ...
        action: "marketplace_loyalty_points_adjusted",
      ...
      const summary = await buildSellerLoyaltySummary(supabase, userId);
    ...
    if (action === "set_status") {
      ...
      const { data, error } = await supabase.rpc("mmd_marketplace_loyalty_set_account_status", {
        p_admin_user_id: session.userId,
        p_seller_user_id: userId,
        p_status: nextStatus,
        p_reason: reason,
      ...
      await writeAdminAuditServer({
        supabaseAdmin: supabase,
        adminUserId: session.userId,
        action: "marketplace_loyalty_account_status_changed",
        targetType: "loyalty_account",
        targetId: userId,
    ...
    if (action === "cancel_redemption") {
      ...
      const { data, error } = await supabase.rpc("mmd_marketplace_cancel_redemption", {
        p_admin_user_id: session.userId,
      ...
      await writeAdminAuditServer({
        supabaseAdmin: supabase,
        adminUserId: session.userId,
        action: "marketplace_reward_canceled",
        targetType: "marketplace_loyalty_redemption",
