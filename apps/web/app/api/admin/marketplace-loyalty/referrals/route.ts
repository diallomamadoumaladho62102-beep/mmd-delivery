...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (action === "mark") {
      ...
      const { data, error } = await supabase.rpc("mmd_marketplace_referral_mark", {
      ...
      await writeAdminAuditServer({
        ...
        action: "marketplace_referral_marked",
        targetType: "marketplace_referral",
    ...
    if (action === "qualify") {
      const { data, error } = await supabase.rpc("mmd_marketplace_referral_qualify", {
        p_referred_user_id: referredUserId,
      });
      if (error) return json({ ok: false, error: error.message }, 500);
      ...
      await writeAdminAuditServer({
        supabaseAdmin: supabase,
        adminUserId: session.userId,
        action: "marketplace_referral_qualified",
        targetType: "marketplace_referral",
        targetId: referredUserId,
    ...
    if (action === "reject") {
      ...
      await writeAdminAuditServer({
        supabaseAdmin: supabase,
        adminUserId: session.userId,
        action: "marketplace_referral_rejected",
        targetType: "marketplace_referral",
        targetId: referredUserId,
        metadata: { reason },
        request,
