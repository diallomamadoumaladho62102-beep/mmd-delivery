...
function buildRulePatch(
  ...
  if (body.seller_user_id !== undefined) {
    patch.seller_user_id = body.seller_user_id === null ? null : cleanText(body.seller_user_id, 40);
  }
  for (const key of ["starts_at", "ends_at"]) {
    if (body[key] !== undefined) patch[key] = body[key] === null ? null : cleanText(body[key], 40);
  }
  for (const key of ["global_quota", "per_seller_quota"]) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      .from("marketplace_loyalty_rules")
      .select("*")
...
export async function POST(request: NextRequest) {
  try {
    ...
    await writeAdminAuditServer({
      ...
      action: "marketplace_loyalty_rule_created",
      targetType: "marketplace_loyalty_rule",
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    await writeAdminAuditServer({
      supabaseAdmin: supabase,
      adminUserId: session.userId,
      action: "marketplace_loyalty_rule_updated",
      targetType: "marketplace_loyalty_rule",
      targetId: id,
