...
async function notifyClientDelivered(params: {
  ...
  const messages = targets.map((target) => {
    ...
    const copy = pushText("food_delivery_completed", target.locale, {
      pickup: pickupText,
      dropoff: dropoffText,
    });
    return {
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Server-side OTP gate: clients that skip UI verify cannot confirm delivery.
    const dropoffCode = String(body.dropoff_code ?? body.code ?? "").trim();
    const { data: verifyData, error: verifyErr } = await supabaseUser.rpc(
      "verify_order_code",
      {
        p_order_id: orderId,
        p_input_code: dropoffCode,
        p_code_type: "dropoff",
      }
    );
    if (verifyErr) {
    ...
    if (verifyRow?.success !== true) {
      return json(
        { error: verifyRow?.message ?? "invalid_dropoff_code" },
        403
      );
    }
