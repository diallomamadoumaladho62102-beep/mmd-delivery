...
// ✅ Aligné : on préfère "text", sinon fallback "message"
  ...
  const fetchInbox = useCallback(async () => {
    try {
      ...
      if (!uid) {
        ...
        setOrders([]);
        setLastMsgByOrder({});
        Alert.alert("Connexion", "Connecte-toi comme client.");
        return;
      }
      setMe(uid);
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 30 }}>
        {loading ? (
          ...
          <Text style={{ color: "#9CA3AF", marginTop: 12 }}>
            Connecte-toi comme client pour voir tes conversations.
          </Text>
        ) : filtered.length === 0 ? (
          <Text style={{ color: "#9CA3AF", marginTop: 12 }}>Aucune commande trouvée.</Text>
        ) : (
          <View style={{ gap: 14 }}>
            ...
            {inProgressOrders.length === 0 ? (
              ...
              <View style={{ gap: 10 }}>{inProgressOrders.map((o) => <Card key={o.id} o={o} />)}</View>
            )}
            ...
            {deliveredOrders.length === 0 ? (
              ...
              <View style={{ gap: 10 }}>{deliveredOrders.map((o) => <Card key={o.id} o={o} />)}</View>
            )}
          </View>
        )}
      </ScrollView>
