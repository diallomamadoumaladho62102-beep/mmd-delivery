...
// ✅ ONLY 6 working languages (match ALLOWED_LOCALES + resources.ts)
...
export function DriverLanguageScreen() {
  ...
  // ✅ i18next = source de vérité
  useEffect(() => {
    const lang = String(i18n.resolvedLanguage || i18n.language || "en").trim().toLowerCase();
    setLocale(lang || "en");
  ...
  const save = useCallback(
    async (next: LangOption["code"]) => {
      ...
      try {
        ...
        setLocale(prev);
        Alert.alert(t("common.errorTitle", "Error"), e?.message ?? "Unable to change language.");
      } finally {
  ...
  const Option = ({ item }: { item: LangOption }) => {
    ...
    return (
      ...
      >
        ...
        >
          <Text style={{ color: active ? "#22C55E" : "#334155", fontWeight: "900", fontSize: 18 }}>
            {active ? "✓" : "›"}
          </Text>
        </View>
      </TouchableOpacity>
