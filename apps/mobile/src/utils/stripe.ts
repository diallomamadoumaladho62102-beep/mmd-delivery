...
/**
 * Shared PaymentSheet present flow for tip PaymentIntents (food + taxi).
 */
async function presentTipPaymentSheet(params: {
  clientSecret: string;
  ...
  if (init.error) {
    logTechnicalError(`${params.logScope}.initPaymentSheet`, init.error, params.context);
...
function parseTipIntentResponse(d: JsonRecord): {
  alreadyPaid: boolean;
  ...
  if (
    ...
    return { alreadyPaid: true, clientSecret: null, merchantCountryCode: "US" };
  }
  ...
  const merchantCountryCode =
    ...
    "US";
  return { alreadyPaid: false, clientSecret, merchantCountryCode };
}
...
export async function payTaxiTipWithPaymentSheet(params: {
  ...
  if (!assertTipPaymentSheetAvailable()) return false;
  ...
  const data = await postJsonWithRetry<JsonRecord | null>(
    `${apiBase()}/api/stripe/client/create-taxi-tip-payment-intent`,
    accessToken,
    {
  ...
  const parsed = parseTipIntentResponse((data ?? {}) as JsonRecord);
  ...
  return presentTipPaymentSheet({
    clientSecret: parsed.clientSecret,
    merchantCountryCode: parsed.merchantCountryCode,
    logScope: "payments.taxiTip",
    context: { taxiRideId },
  });
}
...
export async function payTipWithPaymentSheet(orderId: string): Promise<boolean> {
  ...
  if (!normalizedOrderId) throw new Error("orderId manquant.");
  if (!assertTipPaymentSheetAvailable()) return false;
  ...
  const data = await postJsonWithRetry<JsonRecord | null>(
    `${apiBase()}/api/stripe/client/create-tip-payment-intent`,
    accessToken,
  ...
  return presentTipPaymentSheet({
    clientSecret: parsed.clientSecret,
    merchantCountryCode: parsed.merchantCountryCode,
    logScope: "payments.tip",
    context: { orderId: normalizedOrderId },
  });
}
