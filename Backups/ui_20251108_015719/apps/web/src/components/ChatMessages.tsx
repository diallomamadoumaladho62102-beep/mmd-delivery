...
export default function ChatMessages({ orderId }: { orderId: string }) {
  ...
  const loadBatch = useCallback(async (initial = false) => {
    try {
      ...
      if (initial) {
        ...
        // première pagination sans initial → rien à faire
        return;
      }

      if (!initial && (from === 0 || from === null)) {
        setHasMore(false);
        return;
      }
          ...
          setProfiles(prev => {
            ...
            (async () => {
              const { data: prof } = await supabase
                ...
                .maybeSingle();
              if (prof) {
                setProfiles(p => ({ ...p, [u]: prof as Profile }));
              }
            })();
            return prev;
          });
  ...
  async function del(row: Row) {
    ...
    // 1) Supprimer l'image si présente (Storage)
    if (row.image_path) {
      const { error: delImgErr } = await supabase.storage
        .from("chat-images")
        .remove([row.image_path]);
      if (delImgErr) {
        alert(`Erreur suppression image: ${delImgErr.message}`);
        // on continue quand même pour supprimer la ligne DB
      }
    }

    // 2) Supprimer la ligne DB via RPC (uuid)
    const { error } = await supabase.rpc("delete_order_message", { p_msg_id: row.id });
    if (error) {
      alert(error.message);
      return;
    }
  ...
  return (
    ...
    >
      ...
      {!hasMore && rows.length > 0 && (
        <div className="text-xs text-gray-400 text-center">— fin —</div>
      )}
